...
param parName string = 'keyvault-${resourceGroup().name}'
...
resource resKeyvault 'Microsoft.KeyVault/vaults@2022-07-01' = {
  ...
  properties: {
    accessPolicies: [
      {
        applicationId: parApplicationID
    ...
    networkAcls: {
      ...
      virtualNetworkRules: [for subnets in parAllowedVNETSubnets: {
          id: '${replace(replace(replace(subnets.id,'!SUBSCRIPTIONID!','${subscription().subscriptionId}'), '!RESOURCEGROUP!', '${resourceGroup().name}'),'!VNET!','${parVNETName}')}'
          ignoreMissingVnetServiceEndpoint: subnets.ignoreMissingVnetServiceEndpoint
      }]
    }

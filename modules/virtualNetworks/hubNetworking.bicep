...
resource resHubVnet 'Microsoft.Network/virtualNetworks@2023-06-01' = {
  ...
  properties: {
    ...
    subnets : [for subnet in parSubnets: (subnet.name == 'AzureBastionSubnet' && !parAzBastionEnabled ) ? {} : {
      name      : subnet.name
      properties: {
...
resource resBastionSubnetRef 'Microsoft.Network/virtualNetworks/subnets@2023-06-01' existing = if (parAzBastionEnabled) {
  parent: resHubVnet
  name  : 'AzureBastionSubnet'
}
...
resource resBastion 'Microsoft.Network/bastionHosts@2023-06-01' = if (parAzBastionEnabled) {
  location: parLocation
  ...
  properties: {
    ...
    enableShareableLink: (parAzBastionSku == 'Standard') ? true: null
    enableFileCopy     : (parAzBastionSku == 'Standard') ? true: null
    ipConfigurations   : [
      {
        name      : 'IpConf'
        properties: {
...
resource resGatewaySubnetRef 'Microsoft.Network/virtualNetworks/subnets@2023-06-01' existing = {
  parent: resHubVnet
  name  : 'GatewaySubnet'
}
...
module modGatewayPublicIp '../publicIp/publicIp.bicep' = [for (gateway, i) in varGwConfig: if ((gateway.name != 'noconfigVpn') && (gateway.name != 'noconfigEr')) {
  name  : 'deploy-Gateway-Public-IP-${padLeft(i,2,'0')}'
  params: {
    parLocation          : parLocation
...
//Minimum subnet size is /27 supporting documentation https://docs.microsoft.com/en-us/azure/vpn-gateway/vpn-gateway-about-vpn-gateway-settings#gwsub
resource resGateway 'Microsoft.Network/virtualNetworkGateways@2023-06-01' = [for (gateway, i) in varGwConfig: if ((gateway.name != 'noconfigVpn') && (gateway.name != 'noconfigEr')) {
  name      : gateway.name
  location  : parLocation
  tags      : parTags
  properties: {
...
resource resAzureFirewallSubnetRef 'Microsoft.Network/virtualNetworks/subnets@2023-06-01' existing = {
  parent: resHubVnet
  name  : 'AzureFirewallSubnet'
}
...
resource resFirewallPolicies 'Microsoft.Network/firewallPolicies@2023-06-01' = if (parAzFirewallEnabled) {
...
resource resHubRouteTable 'Microsoft.Network/routeTables@2023-06-01' = if (parAzFirewallEnabled) {
...
output outSubnets array                = resHubVnet.properties.subnets
